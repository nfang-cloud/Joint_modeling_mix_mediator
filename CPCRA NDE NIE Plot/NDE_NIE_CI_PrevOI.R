...
T1=1
tseq=seq(1,15,by=1)
X=as.matrix(covdata[,c("gender","trt","stratum","hemobl")])
tau=15
n=nrow(X)
...
est = as.data.frame(t(est))
 ...
 cov1=read.table("partIII_covI_real_cd4100_quantile.txt",row.names=1)
...
#######################est1 from the real Estimates###############
 ...
 write.csv(Bres1_new,paste("Bres_Mix_PrevOI_new",as.character(1111+simseed),"batch=",as.character(batch),".csv",sep=""),row.names=FALSE)
}
}
