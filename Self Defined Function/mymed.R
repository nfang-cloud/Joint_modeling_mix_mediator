...
mymed<-function(X,n,est,tseq,B){
  ...
  betax=est$betax
  betaw=est$betaw
  lambda.m=exp(c(est$log_lam_m1,est$log_lam_m2,est$log_lam_m3,est$log_lam_m4))
  lambda.y=exp(c(est$log_lam_y1,est$log_lam_y2,est$log_lam_y3,est$log_lam_y4))
  ...
  delta2=est$delta2
  deltam=est$deltam
  log_varv=est$log_varv
  log_vare=est$log_vare
  log_varu=est$log_varu
  
  sigmav=sqrt(exp(log_varv))
  sigmae=sqrt(exp(log_vare))
  sigmau=sqrt(exp(log_varu))
  ...
  for (b in 1:B){
    ...
    M_T=datagen_M_CD4(X,T0,T1,W0_tmp, W1_tmp, t.jump.w,ui, vi,tau,t.jump.m,
